# =============================================================================
#  bernardo/window_30day_submit.R
#
#  Compare all 5 methods when ONLY the first 30 days (excluding seed day 1)
#  are available for calibration. Evaluates on the full 365-day curve.
#
#  Window: obs_inc[2:31]  (days 2-31, 30 days, no seed day)
#
#  BiLSTM (Bernardo): uses pre-computed "noseed_030d" predictions from
#    bernardo_bilstm_predictions.csv  (run generate_bernardo_predictions.py first)
#
#  ABC / ABC-SMC / NM / DE: calibrate in real-time on the 30-day window.
#
#  STEP 1 — Submit:   Rscript bernardo/window_30day_submit.R
#  STEP 2 — Collect:  Rscript bernardo/window_30day_submit.R --collect
# =============================================================================
...
WIN_START <- 2L    # R index (day 2)
WIN_END   <- 31L   # R index (day 31)
WIN_LEN   <- 30L
WIN_TAG   <- "noseed_030d"

# Reduced calibration budgets (single window, must be fast)
...
# BiLSTM noseed_030d predictions
bl_noseed <- preds_all |> filter(window == WIN_TAG)

if (nrow(bl_noseed) == 0)
  stop("No 'noseed_030d' predictions found. Re-run generate_bernardo_predictions.py first.")

cat(sprintf("BiLSTM noseed_030d predictions: %d sims\n", nrow(bl_noseed)))
...
w30_one_sim <- function(row_idx,
  ...
  # Calibration window: days 2-31 (no seed day)
  obs_win <- obs_inc[win_start:win_end]
  # Evaluation: full curve days 2-365
  obs_cmp <- obs_inc[-1]
...
if (!collect_only) {
  ...
  cat(sprintf("Submitting %d SLURM jobs (30-day window, no seed)...\n", n_sims))
  ...
  cat("  Rscript bernardo/window_30day_submit.R --collect\n\n")
  quit(save = "no")
}
...
cat("Collecting 30-day window results...\n")

job_path    <- file.path(path.expand(SCRATCH), "w30_cmp")
...
if (length(results_raw) == 0) stop("No results collected — jobs may still be running or were cancelled.")

metrics_all <- bind_rows(results_raw) |>
  mutate(method = factor(method, levels = METHOD_ORDER))

write.csv(metrics_all,
          file.path(BERNARDO_DIR, "w30_5method_metrics.csv"),
          row.names = FALSE)
cat("Saved: w30_5method_metrics.csv\n")
...
cat("\n========== 30-Day Window (No Seed) — 5-Method Summary ==========\n")
print(as.data.frame(summary_tbl), digits = 4, row.names = FALSE)

write.csv(summary_tbl,
          file.path(BERNARDO_DIR, "w30_5method_summary.csv"),
          row.names = FALSE)
...
p1 <- ggplot(metrics_all, aes(x = method, y = smape, fill = method)) +
  ...
  labs(title    = "sMAPE: First 30 Days (No Seed) — Bernardo BiLSTM",
...
p2 <- ggplot(metrics_long, aes(x = method, y = value, fill = method)) +
  ...
  scale_fill_manual(values = METHOD_COLORS) +
  labs(title    = "All Metrics: First 30 Days (No Seed) — Bernardo BiLSTM",
       subtitle = sprintf("N=%d sims | calibrate on days 2-31 | evaluate on days 2-365",
                          n_sims),
...
p3a <- ggplot(metrics_all,
  ...
  labs(title = "Predicted vs True beta (30-day window)",
...
p3b <- ggplot(metrics_all,
  ...
  geom_point(size = 2.5, alpha = 0.85) +
  scale_color_manual(values = METHOD_COLORS) +
  labs(title = "Predicted vs True R0 (30-day window)",
...
if (nrow(timing_tbl) > 0) {
  p4 <- ggplot(timing_tbl, aes(x = method, y = mean_time_sec, fill = method)) +
    ...
    scale_y_log10() +
    scale_fill_manual(values = METHOD_COLORS) +
    labs(title    = "Calibration Time — 30-Day Window (No Seed)",
...
cat(sprintf("\nAll 30-day window outputs saved to:\n  %s\n", BERNARDO_DIR))
