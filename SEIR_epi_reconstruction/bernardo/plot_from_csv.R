# =============================================================================
#  bernardo/plot_from_csv.R  — regenerate Bernardo comparison plots from CSVs
#
#  Use when compare_slurm.R --collect has finished, or to regenerate plots
#  from existing bernardo_5method_metrics.csv without re-running SEIR.
#
#  Run:
#    Rscript bernardo/plot_from_csv.R
# =============================================================================
...
metrics_path <- file.path(BERNARDO_DIR, "bernardo_5method_metrics.csv")

if (file.exists(metrics_path)) {

  cat("Loading bernardo_5method_metrics.csv...\n")
  metrics_all <- read.csv(metrics_path)

} else {

  cat("bernardo_5method_metrics.csv not found — reconstructing from method summaries.\n")
  ...
  cat(sprintf("Reconstructed %d rows. Note: BiLSTM curve metrics are NA.\n",
              nrow(metrics_all)))
}
...
cost_path <- file.path(BERNARDO_DIR, "bernardo_5method_cost.csv")
...
cat("Saved: bernardo_5method_all_metrics.png\n")
...
p3a <- ggplot(param_scatter,
  ...
  labs(title = "Predicted vs True beta (Bernardo)",
...
p3b <- ggplot(param_scatter,
  ...
  geom_point(size = 2.5, alpha = 0.85) +
  scale_color_manual(values = METHOD_COLORS) +
  labs(title = "Predicted vs True R0 (Bernardo)",
...
p7 <- ggplot(timing_long, aes(x = method, y = mean_wall_sec, fill = method)) +
  ...
  scale_y_log10() +
  scale_fill_manual(values = METHOD_COLORS) +
  labs(title    = "Computation Time — Bernardo BiLSTM",
