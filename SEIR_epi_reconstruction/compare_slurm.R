# =============================================================================
#  compare_slurm.R  — 5-method SEIR comparison, parallelised via slurmR
#
#  Parallelises the expensive SEIR ensemble loop over test sims.
#  Each SLURM worker handles one sim: runs all 5 methods x METHOD_REPS replicates
#  and returns metrics + curve data.
#
#  STEP 1 — Submit:   Rscript compare_slurm.R
#  STEP 2 — Collect:  Rscript compare_slurm.R --collect
#
#  Prerequisites (must exist before running):
#    abc_seir_summary.csv, abcsmc_seir_summary.csv,
#    nm_seir_summary.csv, de_seir_summary.csv,
...
# =============================================================================
# Load all data on the submit node
# =============================================================================
...
cat(sprintf("Evaluating %d sims x 5 methods x %d reps = %d model runs total\n",
            nrow(test_params), METHOD_REPS,
            nrow(test_params) * 5L * METHOD_REPS))

# Lookup table: predicted (beta, R0) per (sim, method)
beta_tbl <- bind_rows(
...
# =============================================================================
# Worker: one sim -> metrics + curves for all 5 methods
# =============================================================================
...
if (!collect_only) {
  ...
  cat(sprintf("Submitting %d SLURM jobs (%d sims)...\n",
              min(n_sims, 500), n_sims))
  ...
  cat("  Rscript compare_slurm.R --collect\n\n")
  quit(save = "no")
}
...
job_path    <- file.path(path.expand(SCRATCH), "seir_compare")
...
# =============================================================================
# Plots (identical to compare_all_methods.R)
# =============================================================================
...
p3a <- ggplot(param_scatter,
  ...
  labs(title = "Predicted vs True beta", x = "True beta", y = "Predicted beta",
...
p3b <- ggplot(param_scatter,
  ...
  geom_point(size = 2.5, alpha = 0.85) +
  scale_color_manual(values = METHOD_COLORS) +
  labs(title = "Predicted vs True R0", x = "True R0", y = "Predicted R0",
       color = "Method", shape = "Method") +
  theme_bw(base_size = 11) +
  theme(panel.grid.minor = element_blank(), plot.title = element_text(face = "bold"))
...
p4 <- ggplot(curve_long, aes(x = day)) +
  ...
  labs(title    = sprintf("SEIR E->I Incidence: Observed vs %d Methods",
                          length(METHOD_ORDER)),
       subtitle = sprintf("Black = observed | bands = 95%% CI (%d runs) | days 2-365",
                          METHOD_REPS),
